...
本文档由论文 **摘要**、**引言**、**方法** 三部分整理而成，供学位论文 / 期刊投稿与工程落地参考。配套轻量产线代码见 [`lightweight_pipeline/`](../../lightweight_pipeline/README.md)。
...
- `[待填：基准数据集]` — 如 OmniDocBench / 自建业务集
- `[待填：对比方法]` — 基线与消融设置
- `[待填：指标]` — Edit Distance、TEDS、整体 F1 等
- 实验数值须与真实实验一致
...
- 评测分数与模型规格来自 2026 年公开论文与官方文档；投稿前请同步修订表格数值。
- 阈值 N、THRESHOLD、检测阈值默认值可在系统实现章与实验章补充。
- 混合 PDF 支持、高精度本地产线（`mode: high_precision`）、PP-TableMagic 接入留作后续工作。
- 摘要/引言中的「写作说明」表格已省略，详见各源文件。

---

*最后更新：2026-05 · 由摘要、引言、方法三份文档整理合并*
